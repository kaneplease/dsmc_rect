...
//物体の大きさ
    ...
    for(int i = 0; i<sides; i++){
        for(int j = 0; j<2; j++){
            for(int k = 0; k <2; k++){
                body_point[i][j][k] = dum[i][j][k];
            }
        }
    }

//body_point[sides][2][2] = { { { -1.5,0.5 },{ 1.45,0.5 } },
//{ { 1.45,0.5 },{ 1.45,3.5 } },
//{ { 1.45,3.5 },{ 1.5,3.5 } },
//{ {1.5,3.5},{1.5,-3.5} },
//{ { 1.5,-3.5 },{ 1.45,-3.5 } },
...
//{ {1.45,-0.5},{-1.5,-0.5} },
//{ {-1.5,-0.5},{-1.5,0.5} } };
//body_point[sides][2][2] = { { { -0.15,0.05 },{ 0.145,0.05 } },
...
//計算領域の詳細確定
    dx = (xmax - xmin) / double(mx);
    dy = (ymax - ymin) / double(my);
    body_dx = (xbody2 - xbody1) / double(B_mx);
...
//分子の質量
    mass_particle = p0*d*d*1.0 / (r_uni*t0)*mol_mass*0.001 / ns;				//単位はkg
    mass_part_2 = rho*d*d / ns;												//無次元化はする？？

//領域内にある粒子の個数
    nmol = 0;												//<----危ないかも

    dtv = (d*fmin(dx, dy) / (v0 + sqrt(2.0*rgas*t0))) / tref;
    dtc = 0.2*akn;
...
//thitの最小値を求めるだけのゴミ関数
double MyDSMC::min_thit(double thit[4])
{
    double min = 100000.0;
...
double MyDSMC::body_force_and_heat(double part_hit[], double c_x, double c_y, double xbody_force_x[][B_my], double xbody_force_y[][B_my], double xbody_energy[][B_my], double ybody_force_x[][B_mx], double ybody_force_y[][B_mx], double ybody_energy[][B_mx])
{
...
double MyDSMC::line_body_collsion(int mol, double body_point_pair[][2][2], double dtfly, double part_x, double part_y, double part_cx, double part_cy, double part_hit[])
{
...
double MyDSMC::rotation(double theta, double xy[], double after_xy[])
{
    //theta はラジアンで
...
double MyDSMC::angle_vectors(double vect_x, double vect_y)
{
...
double MyDSMC::body_rotate(double theta)
{
...
double MyDSMC::delete_particle_in_body()
{
    for (int m = 1; m <= nmol; m++)
    {
        for (int i = 0; i <= sides; i++)
...
double MyDSMC::calc_torque(double hit_x, double hit_y, double u, double v)
{
...
double MyDSMC::intersec_lines(double a_bef[2], double b_bef[2], double a_aft[2], double b_aft[2], double c[2])
{
...
double MyDSMC::near_point_on_line(double a_bef[2], double b_bef[2], double d[2])
{
    double a1 = a_bef[0];
    double a2 = a_bef[1];
    double b1 = b_bef[0];
...
double MyDSMC::rotate_collsion_line(double dtheta, double sum_torque)
{
...
double MyDSMC::c_new_generate(double a_dash[2], double b_dash[2], double theta_c, double v_part_new[2])
{
...
double MyDSMC::check_particle_in_body()
{
    //長方形の中に粒子があるか，チェックする
    for (int m = 1; m <= nmol; m++)
    {
...
//double MyDSMC::simple_rotate_collision(double dtheta)
//{
//	if ((dtheta > -1.0e-8) && (dtheta < 1.0e-8))
//	{
//		return 0;
